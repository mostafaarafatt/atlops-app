...
use Modules\Installer\Entities\Utilities\Installer;
use Session;
...
class InstallerCommand extends Command
{
    ...
    const OPT_ADMIN_NAME = 'admin-name';
    const OPT_ADMIN_EMAIL = 'admin-email';
    const OPT_ADMIN_PHONE = 'admin-phone';
    ...
    protected $signature = 'install
                            ...
                            {--locale=ar : Language used in the app}
                            ';
    ...
    public function handle()
    {
        ...
        Installer::createUser($this->admin_name, $this->admin_email, $this->admin_phone, $this->admin_password, $this->locale);
    ...
    private function prompt()
    {
        ...
        if (empty($this->admin_name)) {
            $this->admin_name = $this->ask('What is the admin name?', 'admin');
        }

        if (empty($this->admin_email)) {
            $this->admin_email = $this->ask('What is the admin email?', 'admin@demo.com');
        }

        if (empty($this->admin_phone)) {
            $this->admin_phone = $this->ask('What is the admin phone?', '987654321');
        }
    ...
    {
        $this->db_host = $this->db_host ?: $this->option(self::OPT_DB_HOST);
        $this->db_port = $this->db_port ?: $this->option(self::OPT_DB_PORT);
        $this->db_name = $this->db_name ?: $this->option(self::OPT_DB_NAME);
        $this->db_username = $this->db_username ?: $this->option(self::OPT_DB_USERNAME);
        $this->db_password = $this->db_password ?: $this->option(self::OPT_DB_PASSWORD);
